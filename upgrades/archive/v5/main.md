...
attack-system rework in [archive/v4/main.md](../v4/main.md), shipped
in full on 2026-08-14; it followed
[archive/v3/main.md](../v3/main.md) (5 steps), which followed
[archive/v2/main.md](../v2/main.md) (15 steps), which followed the
original 8-step plan in [archive/main.md](../main.md)).
...
**Progress: all 10 steps (30–39) shipped 2026-08-19 — wave complete.**
(design tokens in
...
notifications → a real bot-battle History row → Leaderboard); step 39 via
a dead-CSS sweep (cross-referenced every removal against live usage), a
cross-page hardcoded-color cleanup, a 47-screenshot theme×breakpoint
matrix across all 13 routes with a programmatic horizontal-scroll check
on every load, and a full reconciliation of both design docs against
what actually shipped. See each step file's End state notes.
...
- **`.avatar-sm` (Friends, step 38) is a smaller sibling of `.avatar-lg`
  ...
  scale.
- **`WelcomeDialog` and both toast content components needed zero code
  changes in step 39** — a third recurrence of the "page wiring turns out
  to be pure verification" pattern first noticed in step 36: all three
  already rendered entirely through shared classes (`Modal`, `.btn-
  primary`, `.toast-invite`/`.toast-action`) restyled back in steps
  30/31, with nothing cluster-specific left to touch.
- **Dead-CSS removal in step 39 was scoped to this wave's own leftovers**,
  cross-referenced against live `.tsx` usage via grep before deleting
  anything — `.filter-row`/`.stat-filter-row`, `#btn-random-rival`, the
  full `.stat-row`/`.stat-bar-track`/`.stat-bar-fill` family (superseded
  by `SegmentedMeter` in step 34), and two dead `<h3>` selectors
  (`#card-stats h3`/`#online-setup h3`, both converted to `CardTab`).
  Not a full audit of everything predating this wave.
- **Three hardcoded hex colors that predated this wave got tokenized in
  step 39**: `.auth-error`/`.nav-link-badge` → `var(--bad)`, and
  `.auth-switch a`/`.dashboard-note a` → `var(--info)` per
  `DESIGN_SYSTEM.md` §1's explicit link mapping. `.nav-link-badge` also
  picked up the `--accent-ink` pairing (third recurrence of that fix,
  after step 31's status badges and step 35's move buttons) since `--bad`
  runs bright in dark theme and the badge previously used literal white.
- **`DESIGN_SYSTEM.md`'s reconciliation pass (step 39) found exactly two
  real behavioral deviations from the original spec**, both correct
  judgment calls made earlier in the wave and now documented rather than
  left silently stale: base-stat meters keep their pre-existing fixed
  per-stat color instead of the Pokémon's type color (§5 — changing this
  would have been a data/behavior change disguised as a restyle), and the
  battle log never grew per-line win/hit highlighting since it's a single
  `<pre>` block, not per-line spans (§6). Everything else in both design
  docs matched what actually shipped.
- **A Git Bash MSYS path-conversion quirk silently mangled a dynamic
  route argument during step 39's 13-route matrix** (`/friends/<uuid>`
  got rewritten mid-command into a bogus Windows path), causing one
  `page.goto()` to fail and leave the browser showing the previous page —
  not an app bug, confirmed by re-running that one route in isolation
  with `MSYS_NO_PATHCONV=1`. Worth remembering for any future Playwright
  script invoked through this shell with a leading-slash argument.

...
([archive/v2/main.md](../v2/main.md), itself built on the original 8-step
plan at [archive/main.md](../main.md)) shipped in full, including its own
post-plan fixes (starters made untradeable, a UI-polish pass, and the
welcome-lootbox/onboarding dialog — see the bottom of
[archive/v2/main.md](../v2/main.md) for those). This wave was five
smaller, independent items: a tooltip hover-area bug, a real
persistent-notifications feature (today's friend/battle-invite toasts
vanish on refresh with no way to recover them), a match-history/dashboard
display bug (online opponents show as a generic "another player" instead
of their actual name in one of the two places that show match history),
two new status effects (Burn and Freeze), and a stale-inventory bug where
a lootbox opened mid-session didn't show up as a Team Picker option on
the rematch that followed. All five shipped in full.
...
independent items (19, 20) picked up afterward. None blocked any other;
they could have shipped in any order.
...
- **Step 20's bug (newly-opened lootbox Pokémon missing from the Team
  Picker on rematch) turned out to affect both battle UIs, not just the
  one reported.** `OnlineBattle.tsx`'s Rematch and `BattleArena.tsx`'s
  "Change Team" both used their `inventory` prop directly — a
  server-render snapshot from page load — so a lootbox opened mid-session
  never made it back into the list either component offered. Fixed in
  both by holding `inventory` in local state and appending the
  newly-opened Pokémon to it the moment the open-lootbox call returns.

...
## The mana-to-uses migration (shipped in full — steps 40–47, 2026-09-11)
...
| 47 | Docs pass + roster-wide validation | [47-docs-and-roster-validation.md](47-docs-and-roster-validation.md) | 42, 43, 44, 46 | **Shipped** |
...
- **Step 47's docs sweep found 5 more stale mana/MP references** beyond
  what steps 40/43/45 had already caught — 3 code comments claiming
  `resolveTeamRound`/`TeamState`/`FighterState` still track "HP/MP" (now
  `moveUses`), and, more notably, the app's actual page `<meta
  name="description">` in `app/layout.tsx` still advertised "a
  mana-based battle arena" — real user/SEO-facing text, not a comment.
  All fixed. Roster-wide validation (1,667 rolls across all 18 types) and
  battle-exhaustion validation (360 battles across 1v1/3v3-vs-bot-shaped/
  3v3-online-shaped) both passed with zero failures; a real-production-
  data regression across all 69 current rows (21 starter + 48 non-starter)
  also passed. The plan's one item this environment genuinely can't
  perform — a real browser battle, no GUI exists here — is called out
  explicitly as an open item in step 47's own file rather than silently
  marked done; every other environment-appropriate substitute was run.
